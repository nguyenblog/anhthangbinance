...
def run_strategy_1h():
    logging.info("=========== Bắt đầu chu kỳ quét 1H ===========")
    ...
    summary = f"📊 Tổng kết [1H]:\n🔍 Đã kiểm tra {scanned}/{total} coin.\n✅ Có {matched} coin thỏa tất cả điều kiện."
    send_telegram_message(summary, config.TELEGRAM_CHAT_ID, config.TELEGRAM_BOT_TOKEN)
    print("\n" + summary)
    logging.info("=========== Kết thúc chu kỳ quét 1H ===========\n")

if __name__ == "__main__":
    logging.info(f"Bot khởi động cho khung 1H.")
    
    if config.TIME_INTERVAL_UNIT == "minutes":
        schedule.every(config.TIME_INTERVAL).minutes.do(run_strategy_1h)
        logging.info(f"Bot 1H sẽ chạy mỗi {config.TIME_INTERVAL} phút.")
    elif config.TIME_INTERVAL_UNIT == "hours":
        schedule.every(config.TIME_INTERVAL).hours.do(run_strategy_1h)
        logging.info(f"Bot 1H sẽ chạy mỗi {config.TIME_INTERVAL} giờ.")
    else:
    ...
    while True:
        ...
        # Tính toán thời gian còn lại đến lần chạy tiếp theo
