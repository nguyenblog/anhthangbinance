...
# Chu kỳ chạy bot riêng cho 1D
TIME_INTERVAL = config.TIME_INTERVAL_1D
TIME_INTERVAL_UNIT = config.TIME_INTERVAL_UNIT_1D
...
def run_strategy_1d(): # LƯU Ý: Đổi tên hàm chạy chiến lược
    ...
    for batch_start in range(0, total, batch_size):
        ...
        for symbol in batch:
            try:
                ...
                if passed:
                    matched += 1
                    extra["symbol"] = symbol
                    message = format_telegram_message([extra], timeframe="1D")
        ...
        if matched_coins_info:
            message = format_telegram_message(matched_coins_info, timeframe="1D")
            send_telegram_message(message, config.TELEGRAM_CHAT_ID, config.TELEGRAM_BOT_TOKEN)
        time.sleep(1)

    if matched == 0:
        send_telegram_message("❗[1D] Không có đồng coin nào thỏa điều kiện.") # LƯU Ý: Thay đổi timeframe
...
if __name__ == "__main__":
    ...
    if TIME_INTERVAL_UNIT == "minutes":
        schedule.every(TIME_INTERVAL).minutes.do(run_strategy_1d)
        logging.info(f"Bot 1D sẽ chạy mỗi {TIME_INTERVAL} phút.")
    elif TIME_INTERVAL_UNIT == "hours":
        schedule.every(TIME_INTERVAL).hours.do(run_strategy_1d)
        logging.info(f"Bot 1D sẽ chạy mỗi {TIME_INTERVAL} giờ.")
    elif TIME_INTERVAL_UNIT == "days":
        schedule.every(TIME_INTERVAL).days.do(run_strategy_1d)
        logging.info(f"Bot 1D sẽ chạy mỗi {TIME_INTERVAL} ngày.")
    else:
        logging.error("Đơn vị thời gian không hợp lệ. Vui lòng kiểm tra TIME_INTERVAL_UNIT_1D trong .env")
        exit()
